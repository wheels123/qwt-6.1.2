...
QwtPointCusArrayData::QwtPointCusArrayData(
        const QVector<QwtPointCus::PointStyle> &type, const QVector<int> &id , const QVector<double> &x, const QVector<double> &y ,const QVector<double> &phi):
    d_type( type ),
    d_id( id ),
    d_x( x ),
    d_y( y ),
...
*/
QwtPointCusArrayData::QwtPointCusArrayData(
        const QwtPointCus::PointStyle *type ,const int *id,  const double *x, const double *y, const double *phi,size_t size)
{
    d_type.resize( size );
    ::memcpy( d_type.data(), type, size * sizeof( QwtPointCus::PointStyle ) );

    d_id.resize( size );
    ::memcpy( d_id.data(), id, size * sizeof( int ) );
    ...
    d_phi.resize( size );
    ::memcpy( d_phi.data(), phi, size * sizeof( double ) );
}
...
/*!
  ...
  \return Bounding rectangle
*/
QRectF QwtPointCusArrayData::boundingRect() const
{
    if ( d_boundingRect.width() < 0 )
        d_boundingRect = qwtBoundingRect( *this );
...
//! \return Size of the data set
size_t QwtPointCusArrayData::size() const
{
    return qMin( d_x.size(), d_y.size() );
}
...
/*!
  ...
  \return Sample at position i
*/
QwtPointCus QwtPointCusArrayData::sample( size_t index ) const
{
    return QwtPointCus( d_type[int( index )], d_id[int( index )] ,d_x[int( index )], d_y[int( index )], d_phi[int( index )] );
}
//! \return Array of the x-values
const QVector<QwtPointCus::PointStyle> &QwtPointCusArrayData::typeData() const
{
    return d_type;
}

//! \return Array of the x-values
const QVector<int> &QwtPointCusArrayData::idData() const
{
    return d_id;
}

//! \return Array of the y-values
const QVector<double> &QwtPointCusArrayData::yData() const
{
    return d_y;
}
//! \return Array of the x-values
const QVector<double> &QwtPointCusArrayData::xData() const
{
    return d_x;
}

//! \return Array of the y-values
const QVector<double> &QwtPointCusArrayData::phiData() const
{
    return d_phi;
}
