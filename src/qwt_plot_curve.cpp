    ...
    const QRectF &canvasRect, int from, int to ) const
{
    ...
    if ( qwtVerifyRange( numSamples, from, to ) > 0 )
    {
        ...
        int realEnd=to;
        if(d_data->style==NetWorkLines)
        {
            realEnd=(sqrt(4*(to-from+1)+1)-1)/2-1;
        }
        if ( d_data->symbol &&
...
/*!
  ...
  \sa CurveAttribute, setCurveAttribute(),
      draw(), drawCurve(), drawDots(), drawLines(), drawSticks()
*/
void QwtPlotCurve::drawNetWorkLines( QPainter *painter,
    const QwtScaleMap &xMap, const QwtScaleMap &yMap,
    const QRectF &canvasRect, int from, int to ) const
{

    int pointNum=(sqrt(4*(to-from+1)+1)-1)/2;
    int pointRealEnd=from+pointNum-1;
    int netDataStart=pointRealEnd+1;
    const QColor color = painter->pen().color();
    ...
    if ( doFill )
    {
        ...
        QPolygonF points = mapper.toPointsF(
            xMap, yMap, data(), from, pointRealEnd );

        QwtPainter::drawPoints( painter, points );
        int i,j;
        for ( i = from ; i <= pointRealEnd-1; i++ )
        {
            for ( j = i+1 ; j <= pointRealEnd; j++ )
            {
                int netId=netDataStart+pointNum*i+j;
                QPointF netConnected=series->sample( netId );
                if(netConnected.x()>0)
                {
                    ...
                    b.setX(xj);
                    b.setY(yj);
                    QwtPainter::drawLine(painter,a,b);
                }
            }
