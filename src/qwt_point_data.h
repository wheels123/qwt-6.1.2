...
class QWT_EXPORT QwtPointCusArrayData: public QwtSeriesData<QwtPointCus>
{
public:
    QwtPointCusArrayData(const QVector<QwtPointCus::PointStyle> &type, const QVector<int> &id , const QVector<double> &x, const QVector<double> &y ,const QVector<double> &phi );
    QwtPointCusArrayData(const QwtPointCus::PointStyle *type ,const int *id,  const double *x, const double *y, const double *phi,size_t size );

    virtual QRectF boundingRect() const;

    virtual size_t size() const;
    virtual QwtPointCus sample( size_t i ) const;

    const QVector<double> &xData() const;
    const QVector<double> &yData() const;
    const QVector<double> &phiData() const;
    const QVector<QwtPointCus::PointStyle> &typeData() const;
    const QVector<int> &idData() const;
private:
    QVector<double> d_x;
    QVector<double> d_y;
    QVector<double> d_phi;
    QVector<QwtPointCus::PointStyle> d_type;
    QVector<int> d_id;
};
