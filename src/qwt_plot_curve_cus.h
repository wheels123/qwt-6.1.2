...
#ifndef QWT_PLOT_CURVE_CUS_H
#define QWT_PLOT_CURVE_CUS_H
...
/*!
  ...
  with in QwtPlotCurveCus::Lines style and no symbols.
  You can change this by calling
  setPen(), setStyle() and setSymbol().</dd>
  <dt>b) Connect/Assign data.</dt>
  <dd>QwtPlotCurveCus gets its points using a QwtSeriesData object offering
  ...
  the points inside ( like QStandardItemModel ). QwtPlotCurveCus also offers
...
*/
class QWT_EXPORT QwtPlotCurveCus:
    public QwtPlotSeriesItem, public QwtSeriesStore<QwtPointCus>
{
public:
    /*!
        Curve styles.
    ...
    enum CurveStyle
    {
        ...
        */
        ...
        /*!
           ...
           depending on the QwtPlotCurveCus::Inverted attribute.
        */
        Steps,
        ...
        /*!
           ...
           as a curve in QwtPlotCurveCus::NoStyle style and a symbol
           painting a point.
        */
        Dots,

        /*!
           Styles >= QwtPlotCurveCus::UserCurve are reserved for derived
           classes of QwtPlotCurveCus that overload drawCurve() with
    ...
    */
    ...
    {
        /*!
           For QwtPlotCurveCus::Steps only.
           Draws a step function from the right to the left.
         */
        Inverted = 0x01,

        /*!
          Only in combination with QwtPlotCurveCus::Lines
          A QwtCurveFitter tries to
          interpolate/smooth the curve, before it is painted.

          \note Curve fitting requires temporary memory
          for calculating coefficients and additional points.
          If painting in QwtPlotCurveCus::Fitted mode is slow it might be better
          to fit the points, before they are passed to QwtPlotCurveCus.
         */
        Fitted = 0x02
    };
    ...
    {
        /*!
          QwtPlotCurveCus tries to find a color representing the curve
          and paints a rectangle with it.
         */
        LegendNoAttribute = 0x00,

        /*!
          If the style() is not QwtPlotCurveCus::NoCurve a line
    ...
    */
    ...
    explicit QwtPlotCurveCus( const QString &title = QString::null );
    explicit QwtPlotCurveCus( const QwtText &title );

    virtual ~QwtPlotCurveCus();
    ...
    void setSamples( const QVector<QwtPointCus::PointStyle> &typeData, const QVector<int> &idData ,const QVector<double> &xData, const QVector<double> &yData ,const QVector<double> &phiData);
#endif
    void setSamples( const QVector<QwtPointCus> & );
    void setSamples( QwtSeriesData<QwtPointCus> * );
...
protected:
    ...
    virtual void drawNetWorkLines( QPainter *p,
        const QwtScaleMap &xMap, const QwtScaleMap &yMap,
        const QRectF &canvasRect, int from, int to ) const;
private:
    class PrivateData;
...
//! boundingRect().left()
inline double QwtPlotCurveCus::minXValue() const
{
    return boundingRect().left();
}

//! boundingRect().right()
inline double QwtPlotCurveCus::maxXValue() const
{
    return boundingRect().right();
}

//! boundingRect().top()
inline double QwtPlotCurveCus::minYValue() const
{
    return boundingRect().top();
}

//! boundingRect().bottom()
inline double QwtPlotCurveCus::maxYValue() const
{
    return boundingRect().bottom();
}

Q_DECLARE_OPERATORS_FOR_FLAGS( QwtPlotCurveCus::PaintAttributes )
Q_DECLARE_OPERATORS_FOR_FLAGS( QwtPlotCurveCus::LegendAttributes )
Q_DECLARE_OPERATORS_FOR_FLAGS( QwtPlotCurveCus::CurveAttributes )
