...
static inline QRectF qwtBoundingRect( const QwtPointCus &sample )
{
    return QRectF( sample.x(), sample.y(), 0.0, 0.0 );
}
...
QRectF qwtBoundingRect(
    const QwtSeriesData<QwtPointCus> &series, int from, int to )
{
    return qwtBoundingRectT<QwtPointCus>( series, from, to );
}
/*!
  \brief Calculate the bounding rectangle of a series subset
...
/*!
   ...
   \param samples Samples
*/
QwtPointCusSeriesData::QwtPointCusSeriesData(
        const QVector<QwtPointCus> &samples ):
    QwtArraySeriesData<QwtPointCus>( samples )
{
}
...
/*!
  ...
  \return Bounding rectangle
*/
QRectF QwtPointCusSeriesData::boundingRect() const
{
    if ( d_boundingRect.width() < 0.0 )
        d_boundingRect = qwtBoundingRect( *this );
